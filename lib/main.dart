...
class _RadialMenuStage extends StatelessWidget {
  ...
  static const double _flowerSweep = math.pi * 0.38;
                ...
                children: [
                  ...
                  for (var i = 0; i < currentNode.children.length; i++)
                    _RadialNode(
                      ...
                      height: nodeHeight,
                      animationIndex: i,
                      siblingCount: currentNode.children.length,
  ...
  List<Offset> _computeTargets({
    ...
    bool fits(List<Offset> positions) {
      for (final p in positions) {
        ...
        if ((p - center).distance < centerClearance) {
          return false;
        }
      }
    ...
    List<Offset> buildPositions(double radius) {
      ...
      for (var ringIndex = 0; ringIndex < rings.length; ringIndex++) {
        ...
        for (var i = 0; i < take; i++) {
          ...
          final sequenceT =
              total == 1 ? 0.5 : points.length / math.max(total - 1, 1);
          final angle = ringStart +
              (ringEnd - ringStart) *
                  _goldenArcProgress(t.clamp(0.0, 1.0).toDouble());
          final logRadius = ringRadius *
              (0.88 + (0.15 * _logSpiralProgress(sequenceT.toDouble())));
          final dx = center.dx + math.cos(angle) * logRadius;
          final dy = center.dy + math.sin(angle) * logRadius;
    ...
    List<Offset> settlePositions(List<Offset> initial) {
      ...
      for (var iteration = 0; iteration < 96; iteration++) {
        ...
        for (var i = 0; i < total; i++) {
          ...
          final fromCenter = positions[i] - center;
          final centerDistance = math.max(fromCenter.distance, 0.01);
          if (centerDistance < centerClearance) {
            deltas[i] += Offset(
                  fromCenter.dx / centerDistance,
                  fromCenter.dy / centerDistance,
                ) *
                ((centerClearance - centerDistance) * 0.35);
          }
  ...
  double _logSpiralProgress(double t) {
    final clamped = t.clamp(0.0, 1.0).toDouble();
    return (math.pow(_goldenRatio, clamped).toDouble() - 1) /
        (_goldenRatio - 1);
  }
...
class _PetalBloomFrame {
  const _PetalBloomFrame({
    required this.position,
    required this.reveal,
    required this.easedReveal,
  });

  final Offset position;
  final double reveal;
  final double easedReveal;
}

_PetalBloomFrame _petalBloomFrame({
  required Offset center,
  ...
  final rawProgress = progress.clamp(0.0, 1.0).toDouble();
  final safeIndex = index.clamp(0, safeTotal - 1).toInt();
  final indexT = safeTotal == 1 ? 0.5 : safeIndex / (safeTotal - 1);
  final maxDelay = backNode ? 0.0 : math.min(0.24, (safeTotal - 1) * 0.045);
  final delay = safeTotal == 1 ? 0.0 : maxDelay * indexT;
  final reveal =
  ...
  final targetRadius = vector.distance;

  if (targetRadius < 0.1) {
    return _PetalBloomFrame(
      position: target,
      reveal: reveal,
      easedReveal: easedReveal,
    );
  }

  final targetAngle = math.atan2(vector.dy, vector.dx);
  final sweep = backNode ? math.pi * 0.16 : _RadialMenuStage._flowerSweep;
  final spiralRadius = targetRadius *
      ((math.pow(_RadialMenuStage._goldenRatio, easedReveal).toDouble() - 1) /
          (_RadialMenuStage._goldenRatio - 1));
  final angle = targetAngle + (sweep * (1 - easedReveal));
  final slide = Offset(
    -math.min(targetRadius * 0.28, 54.0) * (1 - easedReveal),
    (indexT - 0.5) * math.min(targetRadius * 0.22, 40.0) * (1 - easedReveal),
  );
  final position = center +
      Offset(math.cos(angle) * spiralRadius, math.sin(angle) * spiralRadius) +
      slide;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      paint.color = const Color(
        0xFFBFE8C7,
      ).withValues(alpha: 0.58 * frame.easedReveal);
      canvas.drawPath(_goldenBranchPath(center, frame.position, i), paint);
    }
    ...
    if (hasParent && backTarget != null) {
      ...
      canvas.drawPath(
        _goldenBranchPath(center, frame.position, targets.length),
        paint
          ..color = const Color(
            0xFFE4EFE8,
          ).withValues(alpha: 0.35 * frame.easedReveal),
      );
    }
  }
                ...
                child: Container(
                  ...
                  child: Center(
                    child: Padding(
                      ...
                      child: LayoutBuilder(
                        builder: (context, constraints) {
                          final compact = constraints.maxHeight < 66 ||
